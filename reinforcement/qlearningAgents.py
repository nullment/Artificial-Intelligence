...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        updatedQ = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * estimatedQ
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        "*** YOUR CODE HERE ***"
        features = self.featExtractor.getFeatures(state, action)
        q_value_diff = (reward + self.discount * self.getValue(nextState)) - self.getQValue(state, action)

        for feat in features:
            self.getWeights()[feat] += self.alpha * q_value_diff * features[feat]
        #util.raiseNotDefined()
